...
class PositionEmbeddingSine:
    def __init__(self, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        ...
        if scale is None:
            scale = 2 * math.pi
        self.scale = scale
        # 不在这里创建tensor，而是在调用时根据输入张量的设备创建
        self.num_pos_feats = num_pos_feats 
    ...
    def __call__(self, b, h, w, device=None):
        ...
        if self.normalize:
            eps = 1e-6
            y_embed = ((y_embed - 0.5) / (y_embed[:, -1:, :] + eps) * self.scale).to(device)
            x_embed = ((x_embed - 0.5) / (x_embed[:, :, -1:] + eps) * self.scale).to(device)

        # 在调用时创建dim_t张量，确保在正确的设备上
        dim_t = torch.arange(0, self.num_pos_feats, dtype=torch.float32, device=device)
...
class MVANet(nn.Module):  
    def __init__(self):
        super().__init__()
        self.backbone = SwinB(pretrained=False)  # 修改为不加载预训练权重
        emb_dim = 128
    ...
    def forward(self, x):
        ...
        ####
        device = e5.device  # 获取当前张量的设备
        sideout5 = self.sideout5(e5).to(device)
        sideout4 = self.sideout4(e4).to(device)
        sideout3 = self.sideout3(e3).to(device)
        sideout2 = self.sideout2(e2).to(device)
        sideout1 = self.sideout1(e1).to(device)
        #######glb_sideouts ######
        glb5 = self.sideout5(glb_e5).to(device)
        glb4 = sideout4[-1,:,:,:].unsqueeze(0).to(device)
        glb3 = sideout3[-1,:,:,:].unsqueeze(0).to(device)
        glb2 = sideout2[-1,:,:,:].unsqueeze(0).to(device)
        glb1 = sideout1[-1,:,:,:].unsqueeze(0).to(device)
        ####### concat 4 to 1 #######
        sideout1 = patches2image(sideout1[:-1]).to(device)
        sideout2 = patches2image(sideout2[:-1]).to(device)####(5,c,h,w) -> (1 c 2h,2w)
        sideout3 = patches2image(sideout3[:-1]).to(device)
        sideout4 = patches2image(sideout4[:-1]).to(device)
        sideout5 = patches2image(sideout5[:-1]).to(device)
        if self.training:
